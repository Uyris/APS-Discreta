...
\section*{1. Objetivo}
Aplicar os conceitos de Lógica de Primeira Ordem para representar formalmente um cenário fictício, baseado na história de \\textit{Warcraft III: Reign of Chaos} e \\textit{The Frozen Throne}, utilizando predicados, quantificadores e regras de dedução natural. Em seguida, o modelo foi implementado em Prolog para teste de inferências automáticas.

\section*{2. Cenário: O Mundo de Warcraft III}
O universo de Warcraft apresenta diversos agentes e entidades com relações complexas de poder, corrupção e alianças. Nesta modelagem, consideramos personagens principais: \\textbf{Arthas, Illidan, Thrall, Jaina, Sylvanas, Kel'Thuzad, Mal'Ganis, Archimonde, Lady Vashj} e objetos relevantes: \\textbf{Frostmourne, Frozen Throne, Tome of Power}.\newline
...
\midrule
1. & $\forall x (wields(x, frostmourne) \rightarrow corrupted\_by(x, frostmourne))$ & Premissa (P1) \\
2. & $wields(arthas, frostmourne)$ & Premissa (P2) \\
3. & $\forall x (corrupted\_by(x, frostmourne) \rightarrow became(x, lich\_figure))$ & Premissa (P3) \\
4. & $wields(arthas, frostmourne) \rightarrow corrupted\_by(arthas, frostmourne)$ & $\forall$E 1; $x:=arthas$ \\
5. & $corrupted\_by(arthas, frostmourne)$ & MP 2,4 \\
6. & $corrupted\_by(arthas, frostmourne) \rightarrow became(arthas, lich\_figure)$ & $\forall$E 3; $x:=arthas$ \\
7. & $became(arthas, lich\_figure)$ & MP 5,6 \\
\bottomrule
\end{tabular}
\end{center}
...
\textbf{Passo} & \textbf{Fórmula} & \textbf{Justificativa} \\
\midrule
1. & $\forall x\forall y (killed(x,y) \wedge king(y) \rightarrow traitor(x))$ & Premissa (P1) \\
2. & $killed(arthas, king\_terenas)$ & Premissa (P2) \\
3. & $king(king\_terenas)$ & Premissa (P3) \\
4. & $killed(arthas, king\_terenas) \wedge king(king\_terenas)$ & $\wedge$I 2,3 \\
5. & $(killed(arthas, king\_terenas) \wedge king(king\_terenas)) \rightarrow traitor(arthas)$ & $\forall$E 1; $x:=arthas, y:=king\_terenas$ \\
6. & $traitor(arthas)$ & MP 4,5 \\
\bottomrule
\end{tabular}
\end{center}

\section*{6. Implementação em Prolog}
\begin{verbatim}
% war3.pl -- base de conhecimento
human(arthas).
hero(arthas).
king(king_terenas).
...
\section*{7. Consultas e Resultados Esperados}
\begin{enumerate}
  \item ?- corrupted\_by(arthas, frostmourne). \textbf{true.}
  \item ?- became(arthas, lich\_figure). \textbf{true.}
  \item ?- traitor(arthas). \textbf{true.}
  \item ?- became(arthas, lich\_king). \textbf{true.}
  \item ?- enemy\_of(illidan, nightelves). \textbf{true.}
\end{enumerate}

\section*{8. Conclusão}
O modelo proposto representa de forma lógica a transição de personagens e eventos principais de Warcraft III. As inferências automáticas comprovam a coerência da base de conhecimento e a aplicação correta da Lógica de Primeira Ordem e do raciocínio dedutivo em Prolog.

\vspace{0.5cm}
\noindent\textbf{Data de Entrega:} 05/11/2025 \\
\textbf{Repositório GitHub:} (inserir link na entrega)
